...
export default function OrderPage() {
  ...
  const getAllOrder = async (idAcc) => {
    return new Promise((resolve, reject) => {
      CustomersService.getAllOrder(idAcc)
        .then(response => {
          setOrders(response)
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { orderId,  orderItems, status, total ,address} = row;
                    ...
                    return (
                      <Accordion>
                        <AccordionSummary
                          ...
                          sx={{ width: '100%' }}
                        >
                          <TableRow hover key={orderId} tabIndex={-1} sx={{ width: '100%' }} role="checkbox" selected={selectedUser}>

                          <TableCell align="left" sx={{ width: '4%' }} >  &nbsp;</TableCell>

                            <TableCell component="th" scope="row" padding="none" sx={{ width: '23%' }}>
                            ...
                            <TableCell align="left" sx={{ width: '23%' }}>{address}</TableCell>

                            <TableCell align="left" sx={{ width: '23%' }} >
                              <Label color={'success'}>{status}</Label>
                            </TableCell>

                            <TableCell align="center" sx={{ width: '23%' }} >{total}</TableCell>
                        ...
                        <AccordionDetails>
                          <Typography>
                            {orderItems?.map((orderItem) => (
                              <Typography key={orderItem.itemId} variant="subtitle2" >- {orderItem.productName} &nbsp;&nbsp; Số lượng: &nbsp;{orderItem.quantity} &nbsp;&nbsp; Giá: {orderItem.price} &nbsp;&nbsp;Đơn vị:&nbsp; {orderItem.unit}</Typography>
                            ))}
                          </Typography>
                        </AccordionDetails>
                      </Accordion>
