...
import { Breadcrumbs, Button, Container, Grid, Link, OutlinedInput, Stack, Typography, InputAdornment, useTheme, useMediaQuery, Paper } from '@mui/material';
...
function ProductsList() {
  ...
  const getProduct = async (id) => {
    return new Promise((resolve, reject) => {
      CategoryService.getProductByCategory(id)
        .then(response => {
          setProduct(response)
          resolve();
        })
            ...
            <Grid container spacing={0} >
              ...
              <Grid item xs={12}>
                ...
                <Grid container direction="row" spacing={2}>
                  ...
                  {subCategory?.map((item, index) => (
                    <Grid item xs={4}>
                      <Link underline="hover" color="text.primary" component={RouterLink} to={`/list-products/${item?.id}`}>
                        <Paper elevation={3} sx={{ p: 2, borderRadius: 2 }} key={index} >
              ...
              <Grid item xs={12}>
                ...
                <Stack direction="row" spacing={1} justifyContent="space-between" flexShrink={0} sx={{ my: 1 }}>
                  <Typography variant="h5" sx={{mt:1}}>Danh sách sản phẩm </Typography>
                  <div>
                    <ShopFilterSidebar
              ...
              {(product?.length - 1 - limit) > 0 &&
                <Grid item xs={12}>
                  <Stack justifyContent={'center'}>
                    ...
                    <Button sx={{ color: '#000', mt: 3 }} onClick={() => setLimit(matches ? limit + 10 : limit + 8)}>
